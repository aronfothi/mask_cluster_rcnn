...
class ImageTransform(object):
    ...
    4. vertical flip the image (if needed)
    5. pad the image (if needed)
    ...
    def __init__(self,
        ...
        self.to_rgb = to_rgb
        assert size_divisor is None or pad_shape is None 
    ...
    def __call__(self, img, scale, flip=False, v_flip=False, keep_ratio=True):
        ...
        if self.size_divisor is not None:
            ...
            img = mmcv.impad(img, self.pad_shape)
            pad_shape = img.shape
        else:
            pad_shape = img_shape
...
class BboxTransform(object):
    ...
    def __call__(self, bboxes, img_shape, scale_factor, flip=False, v_flip=False):
        gt_bboxes = bboxes * scale_factor
        if flip:
            gt_bboxes = bbox_flip(gt_bboxes, img_shape)
        if v_flip:
            gt_bboxes = bbox_flip(gt_bboxes, img_shape, direction='vertical')
...
class MaskTransform(object):
    ...
    def __call__(self, masks, pad_shape, scale_factor, flip=False, v_flip=False):
