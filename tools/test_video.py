...
def single_test(model, data_loader, show=False, save_path=''):
    ...
    for data in data_loader:
        ...
        with torch.no_grad():
            rescale = not show
            bbox_results, segm_results, mask_clusters = model(return_loss=False, rescale=rescale, **data)
            if isinstance(bbox_results, list):
                for br, sr in zip(bbox_results, segm_results):
                    results.append((br, sr))
            else:
                results.append((bbox_results, segm_results))

        if show:
            if isinstance(bbox_results, list):
                for e, (br, sr, mc) in enumerate(zip(bbox_results, segm_results, mask_clusters)):
                    
                    model.module.show_result(dict(img=[data['img_seq'][e]], img_meta=[data['img_meta_seq'][e]]),
                                        (br, sr, mc),
                ...
                model.module.show_result(data, (bbox_results, segm_results, mask_clusters), dataset.img_norm_cfg,
                                        dataset=dataset.CLASSES,
                                        save_vis = True,
                                        save_path = save_path,
                                        is_video = True)
...
def parse_args():
    ...
    parser.add_argument('--vid_idx', '--list', nargs='+', type=int, default=[])
